...
ccm.perm <- function(dat,iter =999, span = 12,return.raw = F){
  ...
  sub.dat <- na.omit(dat) %>% mutate(across(everything(),~as.numeric(.))) #drop lead/lag NAs and ensure all columns are numeric
  ...
  ### Permuted CCM ###
  d1.ar1 <- stats::arima(stats::as.ts(sub.dat[,2]), order = c(1, 0, 0),
  ...
  perm.df <- data.frame(sub.dat[,3]) %>% rename(!!sym(paste(colnames(sub.dat)[3])) := 1)
  ...
  for (r in seq_len(iter)) {
    ...
    perm.df <- perm.df %>% 
      mutate(!!sym(new_col_name) :=  red.noise.ts(sub.dat[,2],lag=d1.ar1,length=nrow(sub.dat))) # permuted autocorrelated surrogates
  }  
