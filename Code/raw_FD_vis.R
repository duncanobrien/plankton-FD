...
kin.resid <- cbind(phyto.kin.fuzFDs.mth[,c("FDis","FEve","FRic")],all.system.states$kin.mth[,-c(9)])%>%
  mutate(zooFDis =  zoo.kin.fuzFDs.mth[,"FDis"],zooFEve = zoo.kin.fuzFDs.mth[,"FEve"],zooFRic = zoo.kin.fuzFDs.mth[,"FRic"])%>%
  mutate(across(c(density,mvi,zp.ratio),~log(.x)))%>% # log density, mvi and zo.ration to linearise
  mutate(across(-c(date,data.source,res),~zoo::na.approx(.x,na.rm=F,maxgap = 5)))%>%
  mutate(across(-c(date,data.source,res), function(.x){residuals(lm(.x ~ as.numeric(date),na.action=na.exclude)) - 
      c(decompose(ts(.x,frequency = 12),type = "additive")$seasonal)})) %>%
  mutate(across(-c(date,data.source,res),~scale(.x))) # center and scale to unit variance for plotting
...
all.lakes.resid <- rbind(kin.resid,LZ.resid,mad.resid,wind.resid,kas.resid)%>%
  pivot_longer(-c(date,data.source,res),names_to = "metric",values_to = "value")%>%
  mutate(metric.type = ifelse(metric %in% c("FDis","FEve","FRic"),"Phytoplankton FD",
                              ifelse(metric %in% c("zooFDis","zooFEve","zooFRic"),"Zooplankton FD",
                                     ifelse(metric %in% c("env"),"Stress", "State Metric"))),
...
pdf(file="Results/raw_visualisations/raw_resid_vis.pdf",
    width=9, height = 9)
ggplot(all.lakes.resid %>% mutate(metric = ifelse(metric %in% c("zooFDis","zooFEve","zooFRic"),substr(metric,4,7),metric)),aes(x=as.numeric(date),y=value, col = metric)) + 
  geom_point(aes(col=metric),alpha = 0.3,pch =21)+
  geom_smooth(aes(col = metric),method = "gam",formula =y ~ s(x, bs = "tp",k=15),method.args = list(method = "REML"),alpha=1,fill="grey") +
  #geom_path(aes(col=metric))+
  ggh4x::facet_nested(metric.type + metric~data.source,scales = "free",
  ...
  scale_fill_manual(values = c("#7b3294","#c2a5cf","black","#969014","#22B4F5","#a6dba0","#F07589","#008837","#E8E1A2"), guide = 'none')+
  theme_bw() + xlab("Date")+ ylab("Scaled metric value")+
  geom_vline(data=all.lakes.resid %>% mutate(metric = ifelse(metric %in% c("zooFDis","zooFEve","zooFRic"),substr(metric,4,7),metric)) %>% filter(data.source == "Kasumigaura"), 
             aes(xintercept=1997.0), colour="black",linetype="dashed")+
  geom_vline(data=all.lakes.resid %>% mutate(metric = ifelse(metric %in% c("zooFDis","zooFEve","zooFRic"),substr(metric,4,7),metric)) %>% filter(data.source == "Kinneret"), 
             aes(xintercept=1993.0), colour="black",linetype="dashed")+
  geom_vline(data=all.lakes.resid %>% mutate(metric = ifelse(metric %in% c("zooFDis","zooFEve","zooFRic"),substr(metric,4,7),metric)) %>% filter(data.source == "Mendota"), 
             aes(xintercept=2009.0), colour="black",linetype="dashed")+
  #scale_y_continuous(breaks= scales::pretty_breaks(n = 3))+
  coord_cartesian(ylim = c(-2.5,2.5))+ 
  scale_y_continuous(breaks= seq(-2,2,2))+
