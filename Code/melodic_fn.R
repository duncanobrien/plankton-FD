###########################################################################################
###De Bello et al 2016 Oecologia - 'melodic' function for correcting errors in how 
#abundance-weighted MPD and Rao's Q are calculated in existing packages (picante and dbFD)
###########################################################################################
...
melodic <- function(samp,dis,type="both"){
  if(class(samp)[1]!="matrix"){samp <- as.matrix(samp)}
  if(class(dis)[1]!="matrix"){dis <- as.matrix(dis)}
